        ...
        socket.on('chat', function(msg) {
            ...
            if (!m.action){ 
                ...
                m.forEach((message) => {
                    ...
                    var c = container.find('li.' + action);

                    if (c.length >= 100) {
                        container.find('li.' + action + ':last').remove();
                    }

                    var struct = container.find('li.' + action + ':first');

                    if (message.onlineUsers) {
                        $('#onlines').text(message.onlineUsers);
                    }

                    if (struct.length < 1) {
                        // console.log("Could not handle: " + message); // case action is not defined
                        return;
                    }
